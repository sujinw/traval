...
class code{
    ...
    public function __construct($width = '', $height = '', $bgColor = '', $fontColor = '', $codeLen = '', $fontSize = '') {
        $this->codeStr = conf::get('CODE_STR', 'code');
        $this->font = conf::get('CODE_FONT', 'code');
        if (!is_file($this->font)) {
            error("验证码字体文件不存在");
        }
        $this->width = empty($width) ? conf::get("CODE_WIDTH","code") : $width;
        $this->height = empty($height) ? conf::get("CODE_HEIGHT","code") : $height;
        $this->bgColor = empty($bgColor) ? conf::get("CODE_BG_COLOR","code") : $bgColor;
        $this->codeLen = empty($codeLen) ? conf::get("CODE_LEN","code") : $codeLen;
        $this->fontSize = empty($fontSize) ? conf::get("CODE_FONT_SIZE","code") : $fontSize;
        $this->fontColor = empty($fontColor) ? conf::get("CODE_FONT_COLOR","code") : $fontColor;
    }
    /**
     * 生成验证码
     */
